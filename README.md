...
│                                                                  │
│  • 실시간 JSON 스트림 출력                                       │
│  • 도구 사용: Read, Glob, Grep, Bash, Write, Edit 등            │
│  • 이미지 분석 지원                                              │
└─────────────────────────────────────────────────────────────────┘
                              │
          ┌───────────────────┼───────────────────┐
...
```
analyze.py                    Claude CLI                    Terminal
    │                             │                            │
    │──── subprocess.Popen ──────▶│                            │
    │     (stream-json mode)      │                            │
    │                             │                            │
...
**핵심 발견**: Claude CLI는 `--output-format stream-json --verbose` 옵션으로 실시간 JSON 스트림을 출력합니다. 이를 Python에서 readline()으로 읽으면서 파싱하면 실시간 진행 상황을 표시할 수 있습니다.
...
{
  "clickup": {
    "team_id": "25540965"  // ClickUp URL에서 확인
  },
  "claude": {
...
[Tool] Reading: D:\jar-decompiler\tasks\IMX-9326\images\image_0.png...
[Tool] Searching: packages/**/decompiled/**/*.java
[Tool] Grep: JeusContainer
[Claude] JEUS 9 관련 코드를 분석한 결과...
[Tool] Writing: D:\jar-decompiler\tasks\IMX-9326\report.md...
